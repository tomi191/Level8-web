...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
СТЪПКА 4 — ТОН НА ГЛАСА (критично)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Аудиторията е **собственикът на бизнес, който иска такъв продукт** — умен,
технически грамотен, но не developer. Не говори с developer-а. Говори с
founder-а, който ще плати за подобен продукт.

Принцип: **"сложни проблеми, прости думи"**. Запази substance, махни жаргон.

ПИШИ НА БЪЛГАРСКИ. Не смесвай езиците. Изключения: собствени имена на
продукти/технологии (Stripe, Supabase, Next.js), които са brand names.

Second person винаги когато може: "Ако имате такъв проект...", "Вашият
потребител вижда...". Разказът за това, което Level 8 е направил, остава
в first-person plural ("ние построихме", "решихме").

✅ ДА:
- "Stripe промениха API-то си и тихомълком счупи плащанията. Написахме помощна
  функция, която познава и стария, и новия формат. Решено за един ден."
- "Всяка сутрин платформата трябва да генерира прогнози за всички зодии.
  Бюджетът от време не стигаше. Разбихме процеса на независими стъпки."

❌ НЕ:
- "Stripe 2025-09-30.clover API премести current_period_start/end от Subscription
  root в items[0]. Извлякохме getSubscriptionPeriod helper..."
- "Workflow DevKit позволява durable steps per-zodiac, retry independently"

Конкретни правила:
- Без superlatives ("революционно", "game-changing", "world-class") — фактите говорят.
- Без marketing клишета ("scalable", "enterprise-grade", "cutting-edge").
- Кратки изречения. Ако може 8 думи — не 20.
- Всяко tradeoff, проблем, урок да завършва с човешка поанта, не с техническо резюме.

АКО ПРОЕКТЪТ Е СОБСТВЕН НА LEVEL 8 (Vrachka-подобен):
- В challenge.paragraphs[0] започни с: "<Име> е собствен продукт на Level 8.
  Ние сме и идеята, и разработчиците, и операторите."
- В обекта върни допълнително поле `"isOwnProduct": true` (top-level, до slug).
- Tone-ът подчертава: "Ние използваме това всеки ден за собствения си бизнес.
  Знаем какво значи да поддържаш продукт в production, не само да го построиш."

АКО Е КЛИЕНТСКИ ПРОЕКТ:
- Tone-ът е: "Клиентът имаше визия. Построихме го. Оперираме го. Ето какво
  научихме — което вашият проект няма да преживее."
